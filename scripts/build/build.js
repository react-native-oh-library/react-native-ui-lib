...
const BABEL_OPTIONS = `--config-file .\\src\\.babelrc.json --extensions .ts,.tsx --ignore "src\\**\\*.d.ts,src\\native"`;
...
childProcess.execSync('tsc --p tsconfig.build.json', { stdio: 'inherit' });

console.info('## Build src files - convert TS to JS files ##');
childProcess.execSync(`.\\node_modules\\.bin\\babel src --out-dir src ${BABEL_OPTIONS}`, { stdio: 'inherit' });

console.info('## Build lib (native component) files - convert TS to JS files ##');
childProcess.execSync(`.\\node_modules\\.bin\\babel lib --out-dir lib ${BABEL_OPTIONS}`, { stdio: 'inherit' });

console.info('## Build main index file - for lazy load exports ##');
childProcess.execSync(`.\\node_modules\\.bin\\babel .\\src\\index.js -o .\\src\\index.js ${BABEL_INDEX_EXPORTS_OPTIONS}`, { stdio: 'inherit' });
